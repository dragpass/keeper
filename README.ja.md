...
#### `group_transcrypt_for_guest` - 組織トークンを外部ゲスト向けに再暗号化

生の Group DEK で直接暗号化されたトークンを、外部ゲスト共有用に再暗号化します。
その際、平文が拡張機能へ返されることは一切ありません。Keeper は不透明なセッション
ハンドルの背後にある Group DEK をアンラップし、保護されたメモリ内でトークンを
復号し、使い捨ての 32 バイトのゲスト鍵 `K` を新たに生成して `K` で再暗号化
（パスフレーズによる追加強化も可能）し、平文をゼロ化したうえで、ゲスト暗号文と
`K` のみを返します。

出力は管理コンソールのゲストビューア（`app/src/shared/lib/guest-share-crypto.ts`）
とバイト単位で互換です。

- `guest_ciphertext` = `IV(12) ‖ AES-GCM(暗号文‖タグ)` の標準 Base64
- `guest_key` = 生の 32 バイト `K` の Base64URL（パディングなし）
- AES 鍵はパスフレーズがなければ `K` そのもの、ある場合は
  `HKDF-SHA256(ikm = K ‖ PBKDF2-SHA256(passphrase, salt, 200000, 32B), salt = salt, info = "dragpass-guest-v1", 32B)`。

`passphrase` と `passphrase_salt`（標準 Base64、アプリが生成）は、両方送るか
どちらも省くかのいずれかです。
...
レスポンスに平文や生の Group DEK が含まれることは一切ありません。
