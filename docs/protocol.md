...
|Action|Request fields|Response fields|Description|
|---|---|---|---|
|`mls_group_create`|`permit`, `org_id`, `conversation_id`, `client_commit_id` (UUID), `members` (1..32 × `{ account_id, device_id, key_package_b64 (≤8192B) }`, one entry per device, not the caller), `rotation_statements?`, `room_name_plaintext_b64?` (1..256B, UTF-8; rooms only, 0.0.52)|`{ client_commit_id, expected_epoch (0), commit_b64, welcome_b64, welcome_releasable (false), created, generation, name_epoch?, name_iv_b64?, name_ciphertext_b64? }`|Creates the conversation's group at epoch 0, verifies every member (§5.3 plus the credential match), and builds the Add, pending, in one transaction. Idempotent on `client_commit_id`: a retry answers from the stored Commit with `created: false`. A conversation that already holds a group or a pending Commit is `CHAT_STATE_CONFLICT`. (0.0.52) With `room_name_plaintext_b64` the name is resealed for epoch 1 from the pending create and returned as `name_*` (see "Room names" below); a retry reseals it again under a fresh IV.|
...
|`mls_conversation_status`|`permit`, `org_id`, `conversation_id`|`{ epoch, has_group_state, commit_pending, pending_client_commit_id, removal_latch_account_ids, leaf_replacement_latch: [{ account_id, new_signature_key_fp }], needs_rekey }`|Read-only. The confirmed epoch, the pending Commit's id, the accounts a send would be refused for right now (the stored S-1 latch plus this permit's list, judged on the confirmed roster and not written), the same for the leaf-replacement latch (0.0.51; empty, never null), whether the record is latched `NeedsRekey` (then every other field is zero, and the conversation's history is still readable through `mls_decrypt_batch_for_app_display`), and whether a group exists. Lets the app show "참여자 변경 반영 중" before it tries to send. It passes through the same rollback judgement as every read, so a rewind it finds is latched by it. Returns no secret and nothing derived from one.|
...
(0.0.52) `mls_room_name_open` widens the same carve-out once more, as the v1
reveal's `payload_kind=room_name` does: one `plaintext_b64` entry, the same
