...
unjoinable — and a group older than the 30-day window, whose declarations have
all expired, stays joinable for the same reason. Residual limit: a device that
never saw the newer declaration accepts the stale one at Add time, until that
declaration's `not_after`. **All or nothing**:
