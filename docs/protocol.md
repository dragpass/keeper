...
|`mls_key_package_pool_sweep`|`account_id`|`{ dropped, remaining }`|Drops the owner's KeyPackage pool entries whose KeyPackage does not advertise the room roles extension `0xF0D1` (Q11). A Keeper before wave 5 built its KeyPackages without it, and mls-rs refuses to add a leaf that does not support every group context extension, so after an upgrade those KeyPackages could never be added to a room with roles (the adder would see `CHAT_MLS_ROLES_UNSUPPORTED` for a member that has upgraded). The check reads the KeyPackage inside each private entry (`dpmls_key_package_entry_supports_roles`); an entry that cannot be read is dropped too. An entry a join has claimed is kept. Runs under the pool lock, so two processes sweeping at once drop each entry once between them, and a sweep with nothing to drop writes nothing: idempotent. An owner with no seal key has no pool; none is created. `dropped` counts only unsupported entries (expired ones are pruned as on every write and not counted). The server still serves the matching KeyPackages, so when `dropped > 0` the caller discards this device's unconsumed KeyPackages (`DELETE /account/mls-key-packages`) and refills. No gate: it only deletes private keys this device could never use in a room with roles; the account id scopes it. A build without the MLS library is `CHAT_MLS_CAPABILITY_REQUIRED`; a pool that cannot be read or written is `storage_failure`. Not on the MCP surface.|
...
```
dragpass.chat.state|5|<account_id>|<org_id>|<conversation_id>
...
Fifteen items joined with `|`, no trailing newline, the schema slot always `5`
(0.0.55; a v4 or older permit is refused: it verifies against nothing, and its
...
`CHAT_STATE_INVALID_INPUT` and never repaired.
`pending_device_revocations` (0.0.55) is a JSON array of
`{ account_id, device_id }`; in the canonical each entry is
`<account_id>:<device_id>`, joined with `,`, and the empty string when the
array is `[]`. Lowercase UUIDs, sorted by `account_id` then `device_id`, no
duplicates, at most 64, `null` refused, never repaired. The two goldens ariadne
must reproduce:

```
dragpass.chat.state|5|11111111-1111-4111-8111-111111111111|22222222-2222-4222-8222-222222222222|33333333-3333-4333-8333-333333333333|7|3|11|42|55555555-5555-4555-8555-555555555555,66666666-6666-4666-8666-666666666666|77777777-7777-4777-8777-777777777777:66687aadf862bd776c8fc18b8e9f8e20089714856ee233b3902a591d0d5f2925|88888888-8888-4888-8888-888888888888:99999999-9999-4999-8999-999999999999|1788999000|1788999300|1

dragpass.chat.state|5|11111111-1111-4111-8111-111111111111|22222222-2222-4222-8222-222222222222|33333333-3333-4333-8333-333333333333|7|3|11|42||||1788999000|1788999300|1
```

**Pending device revocations latch encryption (0.0.55, design Q13).** An
account that revokes one of its devices signs a revocation of that device's
leaf; the permit's `pending_device_revocations` latches encryption while the
**confirmed** group state still holds a leaf with that `(account_id,
device_id)`, under S-1's rules exactly: judged in the same four group-loading
operations, stored in the record (`device_revoke_latch`), only ever added to by
a permit, and lifted only by the confirmed roster. A send is refused with
`CHAT_MLS_ROTATION_PENDING`. The Remove that lifts it rests on the account's
own signature, never on this list. A server that omits the entry is not
detected, as with S-1.
...
|`CHAT_MLS_COMMIT_UNAUTHORIZED`|(0.0.55) A Commit this device was asked to build carries an Add or a Remove the Commit authority rules do not allow; nothing was built. A received Commit the rules refuse is not this code: it is `CHAT_MLS_ROW_REFUSED` (a sync block, not a latch).|
|`CHAT_MLS_ROW_REFUSED`|(0.0.55, N3) A received Commit the authority rules refuse; nothing was applied. `data` is the sync block (`epoch`, `cause`, `committer_account_id`, `committer_device_id`, `commit_sha256`). The conversation stops at that epoch until a valid Commit for it is applied; it is not latched.|
|`CHAT_MLS_SYNC_BLOCKED`|(0.0.55, N3) A new message was refused because the conversation is stopped at a received Commit this device refused (`sync_blocked`). Nothing was consumed.|
|`CHAT_MLS_STATEMENT_UNVERIFIED`|(0.0.55) A signed statement in `mls_commit_build` (`org_removal_statements`, `leave_statements`, `device_revocations`) does not verify: bad signature, another organization or conversation, a changed admin key, or a revocation older than the leaf it names. Nothing was built.|
|`CHAT_MLS_ROLES_UNSUPPORTED`|(0.0.55) A member's leaf or KeyPackage does not advertise the room roles extension, which a group carrying roles requires of every leaf. That member's Keeper must update. Nothing was built.|
...
|`CHAT_MLS_PEER_UNVERIFIED`|(0.0.55) This device's strict policy (`require_verified_peers`) is on and the operation would bring in, or send to, another account whose pin is not `verified` (no pin counts): a local Add (`mls_group_create`, `mls_commit_build` add / replace / rejoin), a Join, or a send in a conversation that holds one. **Carries `data: { unverified_account_ids }`**. Nothing was built, joined, consumed or written. Reading is never refused, and neither is a received Commit (see "Strict policy in chat").|
...
**Commit authority (0.0.55, design Q3, Q4, Q5 (b), Q13, Q27).** Every Commit
this Keeper builds or applies is judged on what it adds, removes and does to
the room roles, before the library applies it, from the authenticated group
state and from signatures by accounts whose keys this device pins — never on
the server's word alone (policy 5). mls-rs runs with custom `MlsRules`: its
`filter_proposals` admits Adds (the leaf gate judges those), exactly the Removes
the Go side approved for that one operation, and a GroupContextExtensions
proposal that changes nothing but the roles extension; it refuses every other
proposal type. The same rules check the room roles directly from the group
context, for a Commit this device builds and for one it receives alike.

**Room roles (Q3 phase 2).** A room carries its roles in the GroupContext
extension `0xF0D1`, set by its creator at `mls_group_create`:
...
accounts lowercase and sorted, roles `owner` or `admin`, exactly one owner; a
DM carries the `dm` marker and no roles. Every leaf this Keeper makes
advertises `0xF0D1` in its capabilities, which mls-rs requires of every leaf of
a group carrying it: a member whose Keeper predates this cannot be added to such
a group (`CHAT_MLS_ROLES_UNSUPPORTED`). The rules:

- A **roles change** is accepted only from the owner under the current roles.
  Two exceptions: the **migration** of a group without roles, whose first roles
  may be set only by the account holding leaf 0 (the room's creator) naming
  itself owner; and the **ownerless claim**, when the owner holds no leaf any
  more (removed with the org's signed statement): an admin under the current
  roles, or any member when no admin holds a leaf, may make itself owner and
  change nothing else. Roles are never dropped, a DM's never change, and every
  entry must name an account holding a leaf after the Commit.
- An **Add** in a room needs the committer to be owner or admin; in a DM there
...
- **One active device per account**, in every group (room, DM or
  ...
  the Commit (wave 5c, Q14, N1): every account in the candidate tree must
  hold at most one leaf once it is applied. That refuses a new leaf beside one
  the account keeps, two new leaves of one account in the same Commit, a
  re-seat that brings two leaves for the one it removes, and any Commit that
  keeps a duplicate a group already held from before the rule (an empty
  Update included; only a Commit that removes all but one of those leaves is
  accepted). Nothing repairs such a group on its own; its way on is a new
  group. A second device takes the first one's seat (a handover replace, a
  recovery replace, or a rejoin); it never sits beside it. Two entering
  leaves of one account are usually refused
...
- A **Remove** of account A's leaf by committer C is accepted when **R1** A is
  ...
  allows it: the owner removes anybody, an admin anybody but the owner and the
  admins.

**Signed statements.** Three account-key signatures (RSA-PSS SHA-256, the
rejoin request's shape):
...
The Keeper that builds a Remove verifies the statements the request hands it
(`CHAT_MLS_STATEMENT_UNVERIFIED` otherwise, nothing built) and carries them in
the Commit's authenticated data ("Commit evidence" below). The committer's signature covers them, so a
receiver — catching up included — verifies the same bytes without asking the
server, and a server that strips or alters them breaks the Commit. A leave and
a device revocation verify under the account key in the removed leaf's own
declaration in the authenticated tree (pinned when the leaf entered); a
revocation covers only that device's leaf, and only one declared no later than
the revocation. An org removal verifies under the `admin_public_key` it
carries, held to this device's pin for the admin account (TOFU when none,
recorded; a changed key refuses), and must name this permit's organization.

**Commit evidence (0.0.55).** A Commit's MLS `authenticated_data` is empty or
exactly one JSON document:

```
{"v":1,"org_removals":[...],"leaves":[...],"device_revocations":[...],"handovers":[...]}
```

Every list is optional and omitted when empty; a document with all four
empty is not written (the Commit carries no data). `org_removals`, `leaves`
and `device_revocations` are the signed statements above (each ≤64), and
`handovers` the leaf handovers of "Device succession" below (≤32). One strict
decoder reads it for both rules: an unknown field, another `v`, trailing
bytes, a list past its bound or more than 1 MiB refuses the Commit (on
receipt it is blocked (`unauthorized_commit`, a sync block)), so data one rule wrote is never
read as "nothing" by the other. A Commit may carry both kinds at once: a
replace whose Remove also rests on a statement verifies under both rules from
the same bytes. The Keeper that builds a Commit writes one document holding
the statements its request handed in and the handovers of its `replace`
entries, and judges its own build over those bytes exactly as a receiver
will.

- A Commit this device **builds** is judged by the same rules over the change
  the plan would make. `user_initiated: true` gates the app's intent and is
  never authority: a local Add, a role-based Remove and an owner's roles change
  need it; a Remove on a signed statement, a migration and an ownerless claim do
  not. `replace` needs the permit's `pending_leaf_replacements` (as before);
  `rejoin` needs the account's own signed request and a leaf of the account in
  the authenticated tree (below). A refusal is `CHAT_MLS_COMMIT_UNAUTHORIZED`
  and builds nothing. A create is the group's first Commit and is judged by the
  library's rules against the roles it sets.

**What the client cannot know on its own.** Whether somebody left the
organization: a server that serves no statement keeps that account in the
...
received Commit whose Remove rests on it is refused and is blocked
(`unauthorized_commit`, a sync block). A device that was offline for more than 30 days and
...
MLS `authenticated_data` as the `handovers` list of the evidence document
("Commit evidence" above), each `{ account_id, old_device_id,
old_signature_key_fingerprint, new_device_id, new_signature_key_fingerprint,
issued_at, expires_at, signature }` (at most 32). The
...
exactly an evidence document, one signed by any other key (the new leaf included), or
one naming another old or new leaf — is refused and is blocked
(`unauthorized_commit`, a sync block) like any other refusal above. A leaf of A re-added
...
**Strict policy in chat (0.0.55, design Q8 (a) + (ii)).** The device-local
`require_verified_peers` toggle (`peer_key_policy_set`, set only from the
extension options page, never read or written by the server) also governs
this device's MLS operations. "Unverified" is a pin in `tofu` or `rotated`, or
no pin at all; `changed` is refused whatever the policy says.

| | off | on |
|---|---|---|
|Local Add (create, add, replace, rejoin)|allowed if the leaf verifies|refused with `CHAT_MLS_PEER_UNVERIFIED` while any account it brings in is unverified|
|Join from a Welcome|allowed if every leaf verifies|refused while the tree holds an unverified account; the pool entry is kept|
|Send|allowed|refused while the confirmed tree holds an unverified account, naming them|
|Read (display, history)|allowed|allowed|
|A received Commit that brings in an unverified leaf|applied|applied, never refused (refusing it would fork this device off the group); sends are then refused by the rule above until that member is verified or removed|

The send check runs under the conversation lock on the state the send would
encrypt with (`chatstate.SendRequest.Admit`), so no Commit can slip in between
the check and the encryption. An unreadable policy record fails closed. Set by
an organization is future work: Q8 (c) needs a signed organization policy
first, and until then only this per-device setting exists. It is per device
and not per owner, like the wrap policy it is.
...
|`mls_leave_request_sign`|`permit`, `org_id`, `conversation_id`|`{ statement: { conversation_id, account_id, requested_at, signature } }`|(0.0.55) This account's signed request to leave the conversation (see "Signed statements"): the permit's account, now, signed with the account key. Any remaining member's Keeper accepts a Remove of this account that carries it. Writes nothing.|
|`org_member_removal_sign`|`org_id`, `removed_account_id`, `admin_account_id` (this device's account)|`{ statement: { org_id, removed_account_id, admin_account_id, removed_at, signature } }`|(0.0.55) An org admin's signed statement that an account was removed from the organization. `admin_account_id` must be the account of this device's active leaf; signing for another account is `CHAT_STATE_INVALID_INPUT`-class validation. The server stores it and serves it, with the admin's public key, alongside the pending removals. Writes nothing.|
|`mls_device_revoke_sign`|`account_id` (this device's account), `device_id`|`{ statement: { account_id, device_id, revoked_at, signature } }`|(0.0.55) This account's signed revocation of one of its devices' leaves. Writes nothing.|
...
|`peer_key_safety_number`|`owner_account_id`, `account_id` (≠ owner), `public_key` (PEM, the peer key as the directory serves it)|`{ safety_number, safety_number_b64, own_fingerprint, peer_fingerprint }`|(0.0.55, design Q10 (a)) The pairwise safety number: `SHA-256("dragpass.safety_number\|1\|" + a1 + "\|" + fp1 + "\|" + a2 + "\|" + fp2)` where `(a1, fp1)` and `(a2, fp2)` are the owner and the peer with their account key fingerprints (`hex(sha256(PEM bytes))`), sorted by account id, so both sides of the pair compute the same value. The owner's half comes from this Keeper's own key, never from the request. `safety_number` is the value as a big-endian integer mod 10^60, zero-padded to 60 digits (shown as 12 groups of 5); `safety_number_b64` is the 32-byte value, which a QR code carries as the text `DPSN1:` + that Base64. Confirming it goes through `peer_key_pin_verify` with `safety_number_b64`. It shows that two devices hold the same pair of keys, not who is behind either. Carries nothing secret. Owner-checked like the pin actions.|
|`peer_key_pin_forget`|`owner_account_id`, `account_id`|`{ forgotten }`|Discard a pin. The only path that removes one — account reset and logout leave pins alone, so a human's verification survives signing back in on the same device. Idempotent (`forgotten:false` when there was none) and still prunes a stale index entry. (0.0.55, design Q9) Not a way out of `changed`, and the app does not offer it where a key reads `changed`: forgetting would let the next observation take the new key as a first use, overwriting the pin without the re-verification policy 3 requires. The Keeper cannot enforce this on its own, because `changed` is never stored in a pin (the pin keeps the old key and `changed` is a verdict on each observation), so the rule is the app's.|
...
|`peer_key_policy_set`|`require_verified_peers`|`{ require_verified_peers }`|Turn strict mode on or off and report what is now stored. (0.0.55) It also governs chat: see "Strict policy in chat". The field is required: a payload that omits it is `validation_error` rather than a silent switch-off, since that is the one direction this action must not move by accident.|
