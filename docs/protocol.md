...
|`public material`|`publickey`, `recipient_public_key`, `my_public_key`, `other_public_key`, `new_public_key`, `iv_b64`, `ciphertext_b64` (already enc), `challenge_token`, `signature` (Base64 over public token), `fingerprint` / `old_fingerprint` / `new_fingerprint` (hash of a public key), `old_signature` / `new_signature` (statement signatures), `rotation_statements` (public keys + signatures over them), `signature_key` (raw Ed25519 leaf public key), `signature_key_fingerprint` (hash of it)|OK to log|
...
|`reset_device_identity`|_empty_|`{ cleared }`|Local self-recovery: wipe this device's account-scoped key material — active keypair (`keeper_private_key`/`keeper_public_key`), pending keypair (`pending_keeper_private_key`/`pending_keeper_public_key`), `session_code`, `device_key`, the MLS leaf signature key (`mls_leaf_signature_key`, 0.0.43) — plus every owner's chat state (the sealed files, the anchors, and the seal keys), so the user can re-enroll after a server-side account/DB reset. `cleared` = names of slots actually removed (idempotent: `[]` when nothing was present). `server_public_key` is preserved (account-independent trust anchor). Returns no key material.|
...
### MLS leaf declaration and leaf verification (chat v2, 0.0.43–0.0.44)
...
**The active declaration is stored with the key (0.0.44).** The same slot, now
record version `2`, also holds the declaration `mls_leaf_declare` last signed,
as the exact extension payload bytes a leaf carries. One keyring write replaces
key and declaration together, so `rotate` can never leave one without the
other. A version `1` record (0.0.43) still reads but has no declaration, and a
session cannot be opened from it until an `enroll` for the same identity
re-declares the same key. The record is decoded strictly: an unknown field or
trailing bytes make it unreadable.
...
`{ "v": 1, "declaration": { account_id, device_id, signature_key, signature_key_fingerprint, not_before, reason, signature }, "account_public_key": "<PEM>" }`.
...
(`mls-leaf-newest:<owner>:<account>`, `not_before` and fingerprint, next to the
pins), and an *entering* leaf — one an Add brings in, whether this device builds
...
a different key, is refused. Leaves a Welcome's tree already holds get the
binding checks above and **no** freshness check, and never move the record: a
member who rotated still sits in older groups under the old leaf, which the
members accepted when it entered, and refusing it would make every such group
unjoinable. Residual limit: a device that never saw the newer declaration
accepts the stale one at Add time. **All or nothing**:
...
`account_mls_key_packages.key_package VARBINARY(8192)`, and lives 89 days, a
day inside the server's 90-day ceiling on the declared `not_after`. A real one
with an RSA-2048 account key measures 1554 bytes, 1169 of them the extension.

|Action|Request fields|Response fields|Description|
|---|---|---|---|
|`mls_leaf_declare`|`challenge_token`, `server_signature`, `server_key_version?`, `account_id`, `device_id`, `not_before`, `reason`|`{ account_id, device_id, signature_key, signature_key_fingerprint, not_before, reason, signature }`|Create (`enroll`) or replace (`rotate`) this device's leaf key and sign a declaration for it with the account identity key: RSA-PSS SHA-256 over `dragpass.mls.leaf\|1\|<account_id>\|<device_id>\|<signature_key_fingerprint>\|<not_before>\|<reason>` — seven items, six pipes, no trailing newline, pinned byte-for-byte on both sides. **Gated by a purpose-bound challenge.** The server signature over `challenge_token` must verify, and then the token must be exactly `dragpass.mls.leaf.challenge\|1\|<account_id>\|<device_id>\|<nonce>\|<expires_at_unix>` as issued by `POST /account/mls-leaves/challenge` — six fields, that domain, version `1`, `nonce` 64 lowercase hex, `expires_at` a canonical decimal. Its `account_id` and `device_id` must equal the request's. It is refused at `expires_at` with no grace (as the chat permits are), and also when `expires_at` lies more than 300 + 5 seconds ahead: the issuer sets it to issue time + 300, so that is a token issued in the future beyond the 5s skew the chat permits allow on `issued_at`. A signed token for any other purpose — a rotation challenge, say — is refused. All of it is checked before the keyring slot is read, and every refusal is `validation_error`. Single use is the server's: it consumes the nonce with the declaration write, and the Keeper keeps no nonce list. `not_before` more than 300s ahead of the Keeper clock is `validation_error`. `signature_key` is the raw 32-byte public key in Base64; `signature_key_fingerprint` is lowercase `hex(sha256(those 32 bytes))` — **not** the account key formula, which hashes PEM bytes, and computed by a different function that refuses anything but a raw key. `device_id` is the Extension's `X-Device-ID`: the Keeper has no device identity of its own. `enroll` with no stored key mints one; `enroll` for the identity already stored re-declares the same key, so a lost response costs a retry and not a rotation. `rotate` mints a new key and overwrites the slot only after the declaration is signed; with nothing stored it is `not_found`. Since 0.0.44 every success also stores the signed declaration with the key, in the same write, as the active declaration KeyPackages embed; re-declaring the same key replaces the stored declaration. A stored key for a different `(account_id, device_id)` is refused for both reasons — the way out is `reset_device_identity`. There is no revoke: a `rotate` declaration supersedes the previous one. Not on the MCP surface.|
|`mls_key_package_generate`|`permit` (the conversation-state permit), `org_id`, `conversation_id`, `count` (1..32)|`{ key_packages: [{ key_package_b64, not_after }] }`|Produce `count` single-use KeyPackages for this device's declared leaf, each carrying the active declaration in the `0xF0D0` LeafNode extension, for the caller to upload. `not_after` is the real end of each KeyPackage's lifetime (Unix seconds), which is what the upload declares. **Gated like the conversation-state actions**: strict decode, request/permit binding, the permit window, the server signature — `CHAT_STATE_INVALID_INPUT` / `CHAT_STATE_NOT_AUTHORIZED` as they answer — and it reads no conversation state. The permit's `account_id` must be the account the stored leaf key was declared for, or it is `CHAT_STATE_NOT_AUTHORIZED`. No leaf key, or a 0.0.43 key with no stored declaration, is `not_found`; a build without the MLS library is `CHAT_MLS_CAPABILITY_REQUIRED`. A KeyPackage over 8192 bytes fails the call rather than being dropped. **The HPKE private keys stay in the process that produced them** and are discarded when the call returns; see the 0.0.44 row. Not on the MCP surface.|

|Code|Trigger|
|---|---|
|`CHAT_MLS_LEAF_UNTRUSTED`|A leaf that would enter the group is not vouched for by its account: no declaration, a malformed one, a signature that does not verify, a declaration for another account, device or key, a superseded declaration, or an account whose key the pin reports as `changed`. Nothing was applied and nothing was recorded. When the cause is a changed account key, `data` carries `{ observed_fingerprint, pinned_fingerprint }` as `peer_key_changed` does, so the UI can offer design §5.5's three paths. There is no "ignore and continue".|
