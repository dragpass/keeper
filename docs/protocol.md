...
|0.0.52 (current)|`mls_encrypt` seals a local copy; new `mls_mark_sent`; `mls_decrypt_batch_for_app_display` `items[].state`|**A device reads its own sent messages.** `mls_encrypt` seals the plaintext into the local history in the same write as the outbox entry, keyed by `client_message_id`; the new `mls_mark_sent` binds it to the server's seq, idempotently, refusing a seq bound to another entry. A display batch answers the device's own bound messages from history with itself as the sender, and reports one without a copy as `state: "own_without_local_copy"` instead of failing the batch: the only exception to all or nothing. New FFI status `DPMLS_ERR_FROM_SELF` (-5) for mls-rs `CantProcessMessageFromSelf`. Registered count 98 → 99.|
