...
|`CHAT_MLS_WELCOME_UNUSABLE`|(0.0.50) This device holds no private keys for any KeyPackage the Welcome is addressed to: it expired, was already used, or belonged to a leaf `mls_leaf_promote` has since replaced. The invitation cannot be used on this device and no retry changes that: show "this invitation cannot be used; ask to be invited again", and the inviter adds the member again with a KeyPackage of the current leaf. Nothing was written and the pool is unchanged.|
