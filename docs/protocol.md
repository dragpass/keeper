...
|`unwrapgroupdek`|`encrypted_group_dek`|`{ group_dek_b64 }`|RSA-OAEP unwrap with active privkey. **Carve-out** (raw Group DEK crosses IPC): the client-side crypto path. The extension imports the raw Group DEK as a non-extractable Web Crypto key for drag encrypt/decrypt (the client-side Group DEK cache) and zeroizes the field immediately after import. Handle-only paths use `group_session_open` instead.|
