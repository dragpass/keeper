...
|Action|Request fields|Response fields|Description|
|---|---|---|---|
|`credential_http_request`|`group_handle` **or** `encrypted_dek_b64` (exactly one), `iv_b64`, `ciphertext_b64`, `aad_b64`, `target_url`, `method`, `header_template` (placeholders only), `body_b64?`, signed `policy`|`{ status_code, headers (redacted), body_b64 (redacted), truncated }`|Decrypt-to-tool HTTP sink. Verifies the RSA-PSS server policy, enforces its exact target and request shape, blocks private-network SSRF and redirects, injects the decrypted credential locally, and returns only a bounded redacted response.|
...
|0.0.25 (current)|`dek_unwrap_and_encrypt_with_aad`, `credential_http_request` (personal key source)|Extends the Credential Control Plane to personal scope. `dek_unwrap_and_encrypt_with_aad` is the personal-scope sibling of `group_encrypt_with_aad`: it unwraps the device-wrapped personal DEK and seals under it while binding the same canonical AAD shape (`account_id` replaces `org_id`), so a personal sealed credential payload carries the identical swap guard. `plaintext_b64` carved out in `TestNoRawSecretInRequestTypes` like the other encrypt-direction actions. `credential_http_request` now takes **either** a `group_handle` or an `encrypted_dek_b64` — exactly one, enforced in `Validate()`. Only the key source branches (`withCredentialDEK`); all eight safeguards stay in one implementation, since two copies of a security sink are how safeguards drift apart. The device-wrapped DEK is public material like the opaque handle — unreadable without the Keychain device key, which never crosses IPC.|
