...
```
dragpass.chat.state|3|<account_id>|<org_id>|<conversation_id>
  |<watermark_epoch>|<watermark_leaf_index>
  |<watermark_next_handshake>|<watermark_next_application>
  |<pending_removal_account_ids>
  |<issued_at>|<expires_at>|<server_key_version>
```

Thirteen items joined with `|`, no trailing newline, the schema slot always `3`
(a v2 permit is refused: it verifies against nothing, and its JSON lacks a
required key),
...
same fixed span the chat read permit uses. `pending_removal_account_ids` is the
...
applied"; cryptographic exclusion holds from the epoch the removal produced. The request's `org_id` /
...
|`CHAT_STATE_REKEY_REQUIRED`|The stored state is behind the anchor or behind the server's watermark. Sending is locked for this conversation until a new epoch is established. **No retry path and no bypass** — the latch is the feature. Establishing a new epoch is still not possible (see `mls_establish_epoch` under MLS chat below); `chat_state_purge` is the only way out.|
...
|`mls_conversation_status`|`permit`, `org_id`, `conversation_id`|`{ epoch, has_group_state, commit_pending, pending_client_commit_id, removal_latch_account_ids, needs_rekey }`|Read-only. The confirmed epoch, the pending Commit's id, the accounts a send would be refused for right now (the stored S-1 latch plus this permit's list, judged on the confirmed roster and not written), whether the record is latched `NeedsRekey` (then every other field is zero), and whether a group exists. Lets the app show "참여자 변경 반영 중" before it tries to send. It passes through the same rollback judgement as every read, so a rewind it finds is latched by it. Returns no secret and nothing derived from one.|
...
**`mls_establish_epoch` is not here.** It is the design's only normal way out
of a `NeedsRekey` latch (§12.2), and the merged code has no primitive that can
take it safely, so this release does not add one. Three things are missing,
and none of them is a Keeper-local choice:

- **Nothing in `chatstate` clears `NeedsRekey`.** The anchor latch is cleared
  only by `Purge`, which is owner-wide (it takes every conversation of the
  account with it). A per-conversation reset has to replace the record and
  the anchor together while keeping the anchor's generation monotonic, so a
  restored copy of the rewound file cannot be opened into the new group. That
  transaction does not exist.
- **The permit's watermark is per conversation, not per group.** A fresh group
  starts at epoch 0 while the server has already accepted positions at the old
  epoch, so `Anchor.rewound` sees `watermark_epoch > rec.Epoch` and latches the
  new group on its first load. Moving on needs the permit to scope the
  watermark to a group incarnation (or the server to reset it), which is a
  canonical change on both sides.
- **The server design has no path for a second group in one conversation.**
  In design §12.4 `conversation_handshakes` is unique on `(conversation_id, epoch)` and the
  CAS compares against `conversations.current_epoch`; a new group's epoch 0
  and 1 collide with rows the old group already wrote.

Until those exist, `chat_state_purge` remains the only way out of the latch,
and `mls_conversation_status` reports it (`needs_rekey`) so the app can say so
