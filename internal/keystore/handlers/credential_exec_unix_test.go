...
// credential_exec_unix_test.go — HandleCredentialExecRequest guards that need a
// real child process.
//
// Every test here spawns something, so they are build-tagged to the platforms
// where the exec sink exists at all. Windows has no process-group kill, the
// handler refuses the action outright there (credential_exec_windows_test.go
// covers that), and syscall.Kill / /bin/sh do not exist to write these against.
//
