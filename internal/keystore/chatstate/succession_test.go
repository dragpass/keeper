...
func TestHandovers_RoundTripAndRefuseAnythingElse(t *testing.T) {
	...
	ad, err := proto.MLSCommitEvidence{Handovers: []proto.MLSLeafHandover{h.Wire()}}.Encode()
	if err != nil {
		t.Fatal(err)
	}
	back, err := DecodeHandovers(ad)
	if err != nil || len(back) != 1 || !back[0].Same(h) {
		t.Fatalf("round trip = %+v, %v", back, err)
	}
	if none, err := DecodeHandovers(nil); none != nil || err != nil {
		t.Fatalf("no data decodes as %v, %v", none, err)
	}
	statementsOnly := `{"v":1,"leaves":[{"conversation_id":"c","account_id":"a","requested_at":1,"signature":"s"}]}`
	if none, err := DecodeHandovers([]byte(statementsOnly)); none != nil || err != nil {
		t.Fatalf("a document of statements alone decodes as %v, %v; want no handover", none, err)
	}
	for name, data := range map[string]string{
		...
		"empty document":    `{"v":1,"handovers":[]}`,
		"not json":          "handover",
		"bad signature b64": strings.Replace(string(ad), `"signature":"`, `"signature":"!`, 1),
		"too large":         strings.Repeat(" ", proto.MLSCommitEvidenceMaxBytes+1),
	} {
