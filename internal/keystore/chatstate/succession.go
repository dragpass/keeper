...
// Same reports whether two handovers are the same signed statement.
func (h LeafHandover) Same(o LeafHandover) bool {
	return LeafHandoverCanonical(h) == LeafHandoverCanonical(o) && bytes.Equal(h.Signature, o.Signature)
}
...
//
// The handovers ride in the Commit's one evidence document
// (proto.MLSCommitEvidence), next to the signed statements a Remove rests on.
// This file only converts between that wire shape and LeafHandover; the
// document is written and read by proto alone, so the statement rule and this
// one never read the same bytes differently.

// LeafHandoverFromWire reads a handover as it crosses the wire.
func LeafHandoverFromWire(w proto.MLSLeafHandover) (LeafHandover, error) {
	sig, err := base64.StdEncoding.DecodeString(w.Signature)
	if err != nil {
		return LeafHandover{}, ErrHandoverInvalid
	}
	h := LeafHandover{
	...
	if err := h.Validate(); err != nil {
		return LeafHandover{}, err
	}
	return h, nil
}

// Wire is the handover's wire shape.
func (h LeafHandover) Wire() proto.MLSLeafHandover {
	return proto.MLSLeafHandover{
		AccountID: h.AccountID, OldDeviceID: h.OldDeviceID, OldSignatureKeyFP: h.OldFingerprint,
		NewDeviceID: h.NewDeviceID, NewSignatureKeyFP: h.NewFingerprint,
		IssuedAt: h.IssuedAt, ExpiresAt: h.ExpiresAt,
		Signature: base64.StdEncoding.EncodeToString(h.Signature),
	}
}

// DecodeHandovers reads the handovers of a received Commit's authenticated
// data. Empty data, or a document with statements and no handover, is none.
// Data that is not exactly an evidence document is an error: a Commit
// carrying data this device cannot read is refused rather than read as
// carrying nothing.
func DecodeHandovers(data []byte) ([]LeafHandover, error) {
	ev, err := proto.DecodeMLSCommitEvidence(data)
	if err != nil {
		return nil, ErrHandoverInvalid
	}
	if len(ev.Handovers) == 0 {
		return nil, nil
	}
	out := make([]LeafHandover, len(ev.Handovers))
	for i, w := range ev.Handovers {
		if out[i], err = LeafHandoverFromWire(w); err != nil {
			return nil, err
		}
	}
	return out, nil
